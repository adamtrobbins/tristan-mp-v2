...
contains
  ...
  subroutine injectPhoton(step)
    ...
    xg = REAL(global_mesh % sx) * 0.5 + ph_injdistance * SIGN(1.0, random(dseed) - 0.5)
    ...
    ! if (((xg .lt. REAL(global_mesh % sx) * 0.5) .and. (kx .lt. 0.0)) .or. &
    !     ((xg .gt. REAL(global_mesh % sx) * 0.5) .and. (kx .gt. 0.0))) then
    !   kx = -kx
    ! end if
  ...
  subroutine userReadInput()
    ...
    if (ph_index .gt. 0) then
      ...
      ph_injdistance = ph_injdistance * global_mesh % sx
    end if
            ...
            do p = 1, npart_sp
              ...
              if (s .ne. ph_index) then
                ...
                y_glob = REAL(species(s) % prtl_tile(ti, tj, tk) % yi(p) + this_meshblock % ptr % y0) &
                         + species(s) % prtl_tile(ti, tj, tk) % dy(p)
                ! remove photons which are close to either of the boundaries (no buffer necessary)
                if (((boundary_x .ne. 1) .and. &
                          ((x_glob .lt. injector_sx) .or. (x_glob .ge. REAL(global_mesh % sx) - injector_sx))) .or.&
                  & ((boundary_y .ne. 1) .and. &
                          ((y_glob .lt. 2.0 * ds_abs) .or. (y_glob .ge. REAL(global_mesh % sy) - 2.0 * ds_abs))) &
                    ) then
                  species(s) % prtl_tile(ti, tj, tk) % proc(p) = -1
                  cycle
                end if
