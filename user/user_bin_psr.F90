...
contains
  ...
  subroutine userExternalFields(xp, yp, zp,&
    ...
    logical function userEnforceGCA(xi, yi, zi, dx, dy, dz, u, v, w, weight)
      ...
      real :: rr1, rr2
      rr1 = sqrt((real(xi + this_meshblock%ptr%x0) + dx - xc1_g)**2 +&
               & (real(yi + this_meshblock%ptr%y0) + dy - yc1_g)**2 +&
               & (real(zi + this_meshblock%ptr%z0) + dz - zc1_g)**2)
      rr2 = sqrt((real(xi + this_meshblock%ptr%x0) + dx - xc2_g)**2 +&
               & (real(yi + this_meshblock%ptr%y0) + dy - yc2_g)**2 +&
               & (real(zi + this_meshblock%ptr%z0) + dz - zc2_g)**2)
      userEnforceGCA = ((rr1 .lt. psr_radius1 + psr_gca_enforce_rad) .or. (rr2 .lt. psr_radius2 + psr_gca_enforce_rad))
