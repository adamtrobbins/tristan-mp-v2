...
contains
  ...
  subroutine userDriveParticles(step)
    ...
    real :: x_glob, y_glob, z_glob, u, v, w, shell_x1, shell_x2, shell_y1, shell_y2
    integer :: ind, proc, shell_species, shell_step

    call getInput('output', 'shell_x1', shell_x1)
    call getInput('output', 'shell_x2', shell_x2)
    call getInput('output', 'shell_y1', shell_y1)
    call getInput('output', 'shell_y2', shell_y2)
    call getInput('output', 'shell_time', shell_step)
        ...
        do tj = 1, species(s)%tile_ny
          do tk = 1, species(s)%tile_nz
            do p = 1, species(s)%prtl_tile(ti, tj, tk)%npart_sp
                x_glob = REAL(this_meshblock % ptr % x0) + REAL(species(s) % prtl_tile(ti, tj, tk) % xi(p)) + species(s) % prtl_tile(ti, tj, tk) % dx(p)
                y_glob = REAL(this_meshblock % ptr % y0) + REAL(species(s) % prtl_tile(ti, tj, tk) % yi(p)) + species(s) % prtl_tile(ti, tj, tk) % dy(p)
                z_glob = 0.1 !hack

                if (step /= shell_step) then
                  cycle
                end if
                ...
                if (x_glob < shell_x1 * c_omp) then
                  cycle
                end if

                if (x_glob > shell_x2 * c_omp) then
                  cycle
                end if

                if (y_glob < shell_y1 * c_omp) then
                  cycle
                end if

                if (y_glob > shell_y2 * c_omp) then
                  cycle
                end if
              ...
              ! save particle coordinates/velocities
                ...
                call InjectParticleGlobally(s + 4, x_glob, y_glob, z_glob, u, v, w)
            end do
          end do
        end do
      end do
  ...
  elemental subroutine usrSetElPld(q_over_m, u0, v0, w0, over_e_temp, &
    ...
    real, intent(in) :: q_over_m, u0, v0, w0, over_e_temp, &
                        ...
                        bx0, by0, bz0
    real, intent(out) :: incr_pld1, incr_pld2, incr_pld3
    incr_pld1 = w0 * CC; incr_pld2 = 0.0; incr_pld3 = 0.0
  end subroutine
